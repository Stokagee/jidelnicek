...
| AC-4 | This week + Cook summary (§14.3/.5) | T-7.3 (combined) | `api/summary.test.ts`, `screens/CookSummary.test.tsx`, `screens/ThisWeek.test.tsx` (green) | `acceptance/suites/fe_ep7_summary_ui.robot` | admin cook summary shipped (T-7.3); component tests green; browser suite active. Member-facing "who/portion totals" in This-week deferred to BE #53. |
| AC-1, AC-2, AC-3, AC-6 | Dish signup (§14.4) | T-6.3 (combined) | `domain/block.test.ts`, `domain/portions.test.ts`, `api/signups.test.ts`, `dishes/SignupPage.test.tsx` (green) | `acceptance/suites/fe_ep6_signups_ui.robot` (AC-1/2/6) | screen shipped (T-6.3); component tests green; browser suite active (needs `rfbrowser init` + running api/web). AC-3 upsert is server-side (re-submit); covered at BE integration. |
